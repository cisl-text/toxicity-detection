...
class SBICDataset(Dataset):
    def __init__(self, tokenizer, data_dir="./data/SBIC/processed_dataset", mode='dirctr_gold_train', prepared_data=None, export=False, correction_dir=None,correction_size=None):
        ...
        if prepared_data:
            ...
            self.get_data(data_dir, mode, ma_processor, correction_dir, correction_size)

    def get_data(self, data_dir, mode, ma_processor, correction_dir, correction_size):
...
class SBICMTLDataset(Dataset):
    def __init__(self, tokenizer, data_dir="./data/SBIC/mtl_data", mode='lewd',
                 prepared_data=None, export=False, correction_dir=None, correction_size=None):
        """
        dirctr_missed_train：将implicit 的数据标为0 miss micro
        dirctr_corrected_train： 更正错误的样例
        dirctr_checked_train：更正implicit错误的样例
    ...
    def get_MTL_data(data_dir=r'data/SBIC/mtl_data/', split_ratio=0.9, shuffle=False, data="lewd"):
        ...
        train_pos_data, test_pos_data = split_data(pos_data, split_ratio)
        train_non_data, test_non_data = split_data(non_data, split_ratio)

        train_data = train_pos_data  + train_non_data
        test_data = test_pos_data  + test_non_data
